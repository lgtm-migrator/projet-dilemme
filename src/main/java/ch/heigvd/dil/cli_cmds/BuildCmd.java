...
/**
 * Cette classe reproduit la structure du site et convertit les fichiers Markdown
 * en HTML et retourne le résultat dans le dossier "build"
 **/
...
public class BuildCmd implements Callable<Integer> {
  ...
  public Integer call() {
    ...
    int initFiles = 0;
    for (File f: allFiles){
      if (f.getName().equals("index.md") || f.getName().equals("config.json"))
        ++initFiles;
    }
  ...
  /**
   * Explore les fichiers Markdown du dossier courant et les convertit en HTML dans
   * le dossier build
